...
const Input: React.FC<React.InputHTMLAttributes<HTMLInputElement>> = ({className = "", type = "text", ...props}) => {
    return (
        <input
            type={type}
        ...
        disabled:cursor-not-allowed disabled:opacity-50 text-gray-900 ${className}`}
            {...props}
        />
    );
};
